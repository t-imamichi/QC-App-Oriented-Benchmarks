...
def default_noise_model():
    ...
    noise.add_all_qubit_quantum_error(
        depolarizing_error(depol_one_qb_error, 1), ["rx", "ry", "rz"]
    )
    ...
    noise.add_all_qubit_quantum_error(
        depolarizing_error(amp_damp_one_qb_error, 1), ["rx", "ry", "rz"]
    )
    noise.add_all_qubit_quantum_error(
        depolarizing_error(amp_damp_two_qb_error, 2), ["cx"]
    )
    ...
    noise.add_all_qubit_quantum_error(
        reset_error(reset_to_zero_error, reset_to_one_error), ["reset"]
    )

    # Add readout error
    p0given1_error = 0.000
    p1given0_error = 0.000
    error_meas = ReadoutError(
        [[1 - p1given0_error, p1given0_error], [p0given1_error, 1 - p0given1_error]]
    )
...
def set_execution_target(
    ...
    authentication_error_msg = (
        "No credentials for {0} backend found. Using the simulator instead."
    )
    ...
    if provider_backend is not None:
        ...
        if hub == "azure-quantum":
            ...
            if context is not None:
                ...
                session_name = f"QED-C Benchmark Session {session_count}"

            if verbose:
                print(
                    f"... creating session {session_name} on Azure backend {backend_id}"
                )

            # open a session on the backend
            session = backend.open_session(
                name=session_name, job_failure_policy=SessionJobFailurePolicy.CONTINUE
            )
    ...
    elif backend_id == "statevector_estimator":
    ...
    # otherwise use the given providername or backend_id to find the backend
        ...
        if hub == "azure-quantum":
            ...
            if verbose:
                print(
                    f"... creating Azure backend {backend_id} and session {session_name}"
                )
        ...
        ###############################
        # otherwise, assume IBM Cloud or IBM Quantum devices.
        else:
            ...
            if primitive_name not in ["sampler", "estimator"]:
                raise ValueError(
                    f"invalid primitive name: {primitive_name}. Should be 'sampler' or 'estimator'"
                )
            ...
            if this_use_sessions is not None:
            ...
            # otherwise, use Sampler without sessions
            else:
                session = None
                if verbose:
...
def submit_circuit(qc, group_id, circuit_id, shots=100, params=None):
    ...
    if verbose:
        print(
            f'... submit circuit - group={circuit["group"]} id={circuit["circuit"]} shots={circuit["shots"]} params={circuit["params"]}'
        )
    ...
    logger.info(
        f'Submitting circuit - group={circuit["group"]} id={circuit["circuit"]} shots={circuit["shots"]} params={[param[1] for param in params.items()] if params else None}'
    )
...
def execute_circuit(circuit: dict):
    ...
    try:
        ...
        if do_transpile_metrics and use_normalized_depth:
            qc_tr_depth, qc_tr_size, qc_tr_count_ops, qc_tr_xi, qc_tr_n2q = (
                transpile_for_metrics(qc)
            )
        ...
        if (
            backend_exec_options_copy is not None
            and "noise_model" in backend_exec_options_copy
        ):
        ...
        if backend_exec_options_copy is None:
        ...
        if executor:
            ...
            qc = circuit["qc"]
            result = executor(
                qc, backend_name, backend, shots=shots, **backend_exec_options_copy
            )
        ...
        # normal execution processing is performed here
            ...
            if (
                this_noise is not None
                and not sampler
                and backend_name.endswith("qasm_simulator")
            ):
                ...
                if transformer:
                    ...
                    simulation_circuits, shots = invoke_transformer(
                ...
                job = backend.run(
                    simulation_circuits,
                    shots=shots,
                    noise_model=this_noise,
                    basis_gates=this_noise.basis_gates,
                    **backend_exec_options_copy,
                )

                logger.info(
                    f"Finished Running on noisy simulator - {round(time.time() - st, 5)} (ms)"
                )
                if verbose_time:
            ...
            # Initiate execution for all other backends and noiseless simulator
                ...
                if sampler:
                    ...
                    job = backend.run(
                        trans_qc, shots=shots, **backend_exec_options_copy
                    )
    ...
    metrics.store_metric(
        active_circuit["group"], active_circuit["circuit"], "n2q", qc_n2q
    )

    metrics.store_metric(
        active_circuit["group"], active_circuit["circuit"], "tr_depth", qc_tr_depth
    )
    metrics.store_metric(
        active_circuit["group"], active_circuit["circuit"], "tr_size", qc_tr_size
    )
    metrics.store_metric(
        active_circuit["group"], active_circuit["circuit"], "tr_xi", qc_tr_xi
    )
    metrics.store_metric(
        active_circuit["group"], active_circuit["circuit"], "tr_n2q", qc_tr_n2q
    )

    # also store the job_id for future reference
    metrics.store_metric(
        active_circuit["group"], active_circuit["circuit"], "job_id", job.job_id()
    )
...
# Launch execution of one job (pub)
...
def wait_on_job_result(job, active_circuit):
    ...
    while retry_count < 40:
        try:
            ...
            print(
                f'... error occurred during job.result() for circuit {active_circuit["group"]} {active_circuit["circuit"]} -- retry {retry_count}'
            )
            if verbose:
                print(traceback.format_exc())
            time.sleep(15)
    ...
    if result is None:
        print(
            f'ERROR: during job.result() for circuit {active_circuit["group"]} {active_circuit["circuit"]}'
        )
...
def get_job_status(job, active_circuit):
    ...
    while retry_count < 3:
        try:
            ...
            print(
                f'... error occurred during job.status() for circuit {active_circuit["group"]} {active_circuit["circuit"]} -- retry {retry_count}'
            )
            if verbose:
                print(traceback.format_exc())
            time.sleep(15)
    ...
    if status is None:
        print(
            f'ERROR: during job.status() for circuit {active_circuit["group"]} {active_circuit["circuit"]}'
        )
...
def transpile_for_metrics(qc):
    ...
    n1q = 0
    n2q = 0
    if qc_tr_count_ops is not None:
    ...
    if verbose_time:
        print(f"  *** transpile_for_metrics() time = {round(time.time() - st, 5)}")
...
def transpile_and_bind_circuit(
    circuit,
    params,
    backend,
    basis_gates=None,
    ...
    if do_transpile_for_execute:
        ...
        print(f"  ... transpiling with {optimization_level=}")
        trans_qc = transpile(
            circuit,
            backend,
        ...
        if verbose_time:
            print("  ... using cached circuit, no transpile")
    ...
    if params is not None:
        ...
        logger.info(
            f"Binding parameters to circuit: {[param[1] for param in params.items()]}"
        )
        if verbose_time:
    ...
    logger.info(
        f"transpile_and_bind_circuit - {trans_qc_name} {round(time.time() - st, 5)} (ms)"
    )
    if verbose_time:
        print(f"  *** transpile_and_bind() time = {round(time.time() - st, 5)}")
...
def transpile_multiple_times(
    ...
    logger.info(
        f"transpile_multiple_times - {best_idx} {round(time.time() - st, 5)} (ms)"
    )
    if verbose_time:
        print(f"  *** transpile_multiple_times() time = {round(time.time() - st, 5)}")
...
def job_complete(job):
    ...
    logger.info(
        f'job complete - group={active_circuit["group"]} id={active_circuit["circuit"]} shots={active_circuit["shots"]}'
    )
    ...
    metrics.store_metric(
        active_circuit["group"], active_circuit["circuit"], "elapsed_time", elapsed_time
    )
    metrics.store_metric(
        active_circuit["group"], active_circuit["circuit"], "exec_time", exec_time
    )
    ...
    if hasattr(job, "local_job"):
        ...
        if result is not None and result_handler:
            try:
                result_handler(
                    active_circuit["qc"],
                    result,
    ...
    if job.status() == JobStatus.DONE or job.status() == "DONE":
        ...
        if azure_provider is not None and session is not None:
            ...
            exec_time = (
                details.end_execution_time - details.begin_execution_time
            ).total_seconds()
            ...
            startup_time = (
                details.begin_execution_time - details.creation_time
            ).total_seconds()
...
def process_step_times(job, result, active_circuit):
    ...
    if use_sessions:
        ...
        try:
            created_time = datetime.strptime(
                job_timestamps["created"][11:-1], "%H:%M:%S.%f"
            )
            created_time_delta = timedelta(
                hours=created_time.hour,
                minutes=created_time.minute,
                seconds=created_time.second,
                microseconds=created_time.microsecond,
            )
            finished_time = datetime.strptime(
                job_timestamps["finished"][11:-1], "%H:%M:%S.%f"
            )
            finished_time_delta = timedelta(
                hours=finished_time.hour,
                minutes=finished_time.minute,
                seconds=finished_time.second,
                microseconds=finished_time.microsecond,
            )
            running_time = datetime.strptime(
                job_timestamps["running"][11:-1], "%H:%M:%S.%f"
            )
            running_time_delta = timedelta(
                hours=running_time.hour,
                minutes=running_time.minute,
                seconds=running_time.second,
                microseconds=running_time.microsecond,
            )

            # compute the total seconds for creating and running the circuit
            exec_creating_time = (
                running_time_delta - created_time_delta
            ).total_seconds()
            exec_running_time = (
                finished_time_delta - running_time_delta
            ).total_seconds()
            ...
            if verbose:
                print(
                    f'WARNING: incomplete time metrics for circuit {active_circuit["group"]} {active_circuit["circuit"]}'
                )
    ...
    metrics.store_metric(
        active_circuit["group"],
        active_circuit["circuit"],
        "exec_validating_time",
        0.001,
    )
    metrics.store_metric(
        active_circuit["group"], active_circuit["circuit"], "exec_queued_time", 0.001
    )
    metrics.store_metric(
        active_circuit["group"],
        active_circuit["circuit"],
    ...
    if verbose:
        print(
            f"... computed exec times: queued = {exec_queued_time}, creating/transpiling = {exec_creating_time}, validating = {exec_validating_time}, running = {exec_running_time}"
        )
    ...
    for job, circuit in active_circuits.items():
        try:
            ...
            status = get_job_status(
                job, circuit
            )  # use this version, robust to network failure
            # print("Job status is ", status)

        except Exception as e:
            print(
                f'ERROR: Unable to retrieve job status for circuit {circuit["group"]} {circuit["circuit"]}'
            )
            print(f"... job = {job.job_id()}  exception = {e}")
